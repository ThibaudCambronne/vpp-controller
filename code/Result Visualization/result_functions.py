...
def plot_capacity_allocation(results_list, OUT_PATH):
    ...
    plt.tight_layout()
    plt.savefig(
        OUT_PATH / "capacity_allocation_by_node.png", dpi=150, bbox_inches="tight"
    )
    # plt.show()
    plt.close()
    return
...
def plot_battery_dispatch_3d(
    ...
    suffix = ("_norm" if normalize else "") + (
        "_filtered" if filter_small_nodes else ""
    )
    plt.savefig(
...
def plot_transmission_congestion(results_dict, OUT_PATH, filter_small_nodes=False):
    ...
    plt.savefig(
        OUT_PATH / f"battery_thermal_duals_cap{total_cap:.0f}MWh{suffix}.png",
        dpi=150,
        bbox_inches="tight",
        pad_inches=0.5,
    )
...
def plot_node_dispatch_by_capacity(results_list, node, OUT_PATH, normalize=False):
    ...
    for r in sorted_results:
        ...
        ax.plot(
            ...
            markersize=3,
            linewidth=1.5,
            label=f"Cap = {total_cap:.1f}",
        )

    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Hour")
    ax.set_ylabel(
        "% of Node Capacity Dispatched" if normalize else "Charge / Discharge (MW)"
    )
